...
func spawn_room(_position=null, color_param=1):
...
# Need to be sure these arrays match in size with data_pts_per_room list

func room_data_load(map, initial_seed, data_pts_per_room, prob_dists):
...
func _ready():
	...
	print("")
	navigation_load(map, directions)
	room_data_load(map, init_seed, 2, array_mult([[[1, 1], [1, 2]]], 6) )
